...
export const CHANGELOG: ChangelogEntry[] = [
  ...
  },
  {
    ...
    date: "2026-08-12",
    notes: {
      en: "The writing editor gained a structured project format — chapters and scenes as a real document tree with metadata, a corkboard view for reordering by drag, project-wide search with backlinks, 7 built-in themes, 5 starter templates, and a 10-step onboarding tour you can replay anytime. Existing single-file or folder manuscripts migrate automatically. Also fixed: PDF export was silently broken on stock macOS (a font-loading assumption that never matched this platform) — now embeds a proper Unicode font at build time, verified with Cyrillic and Czech text. Built-in theme names weren't translated in the theme picker; the onboarding tour could be permanently dismissed by an accidental click before you'd even seen the button to bring it back; the tour dialog was missing an accessibility label its sibling dialogs already had. Security: HTML export could execute embedded script content from a chapter body instead of showing it as plain text; a corrupted project file with a circular reference could crash the app instead of failing cleanly.",
      ru: "Текстовый редактор получил структурированный формат проекта — главы и сцены как настоящее дерево документов с метаданными, вид «пробковая доска» для перестановки перетаскиванием, поиск по всему проекту с обратными ссылками, 7 встроенных тем, 5 стартовых шаблонов и 10-шаговый обучающий тур, который можно повторить в любой момент. Существующие рукописи (один файл или папка) переносятся автоматически. Также исправлено: экспорт в PDF был незаметно сломан на обычном macOS (предположение о шрифтах, которое никогда не подходило этой платформе) — теперь нужный Unicode-шрифт встраивается при сборке, проверено на кириллице и чешском тексте. Названия встроенных тем не переводились в пикере тем; случайный клик мимо карточки обучающего тура мог необратимо его закрыть ещё до того, как вы увидели кнопку, чтобы вернуть его; у диалога тура не хватало метки доступности, которая уже была у соседних диалогов. Безопасность: экспорт в HTML мог выполнить встроенный скрипт из текста главы вместо того, чтобы показать его как обычный текст; повреждённый файл проекта с циклической ссылкой мог уронить приложение вместо аккуратной ошибки.",
      uk: "Текстовий редактор отримав структурований формат проєкту — глави та сцени як справжнє дерево документів із метаданими, вигляд «пробкова дошка» для перестановки перетягуванням, пошук по всьому проєкту зі зворотними посиланнями, 7 вбудованих тем, 5 стартових шаблонів і 10-кроковий навчальний тур, який можна повторити будь-коли. Наявні рукописи (один файл або папка) переносяться автоматично. Також виправлено: експорт у PDF був непомітно зламаний на звичайному macOS (припущення про шрифти, яке ніколи не підходило цій платформі) — тепер потрібний Unicode-шрифт вбудовується під час збірки, перевірено на кирилиці та чеському тексті. Назви вбудованих тем не перекладалися в пікері тем; випадковий клік повз картку навчального туру міг незворотно закрити його ще до того, як ви побачили кнопку, щоб повернути його; діалогу туру бракувало мітки доступності, яка вже була в сусідніх діалогах. Безпека: експорт у HTML міг виконати вбудований скрипт із тексту глави замість того, щоб показати його як звичайний текст; пошкоджений файл проєкту з циклічним посиланням міг зронити застосунок замість акуратної помилки.",
      cs: "Textový editor získal strukturovaný formát projektu — kapitoly a scény jako skutečný strom dokumentů s metadaty, zobrazení „korková nástěnka“ pro přeskupování přetažením, vyhledávání v celém projektu se zpětnými odkazy, 7 vestavěných motivů, 5 startovních šablon a 10krokovou úvodní prohlídku, kterou lze kdykoli zopakovat. Existující rukopisy (jeden soubor nebo složka) se přenášejí automaticky. Také opraveno: export do PDF byl nenápadně rozbitý na běžném macOS (předpoklad o fontech, který této platformě nikdy neodpovídal) — nyní se při sestavení vkládá správný Unicode font, ověřeno na azbuce a českém textu. Názvy vestavěných motivů se v přepínači motivů nepřekládaly; náhodné kliknutí mimo kartu úvodní prohlídky ji mohlo nevratně zavřít ještě předtím, než jste viděli tlačítko pro její opětovné spuštění; dialogu prohlídky chyběl štítek přístupnosti, který už měly sousední dialogy. Zabezpečení: export do HTML mohl spustit vložený skript z textu kapitoly místo toho, aby ho zobrazil jako obyčejný text; poškozený soubor projektu s cyklickým odkazem mohl shodit aplikaci místo čisté chyby.",
    },
  },
  {
    version: "0.4.8",
